...
namespace IO.Eventuate.Tram.UnitTests.Messaging.Producer
{
	public class AbstractMessageProducerTests
	{
		...
		public void Send_SimpleMessage_MessageHeadersAreApplied()
		{
			...
			// Assert
			Assert.That(sendMessage, Is.Not.Null);
			Assert.That(sendMessage.GetRequiredHeader(MessageHeaders.Id), Is.Not.Null);
			Assert.That(sendMessage.GetRequiredHeader(MessageHeaders.Destination), Is.Not.Null);
			Assert.That(sendMessage.GetRequiredHeader(MessageHeaders.Date), Is.Not.Null);
		}
	}
}