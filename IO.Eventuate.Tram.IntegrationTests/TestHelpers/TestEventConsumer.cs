	...
	public class TestEventConsumer
	{
			...
			IServiceProvider serviceProvider, CancellationToken cancellationToken)
		{
			...
			_logger.LogDebug("Got message TestMessageTypeDelay with id={} and value={}", @event.EventId,
				@event.Event.ToString());
			await File.AppendAllTextAsync(IntegrationTestsBase.PingFileName,
				$"Received event '{@event.Message.Payload}' with value '{@event.Event.Value}'\n", cancellationToken);
			...
			await File.AppendAllTextAsync(IntegrationTestsBase.PingFileName,
				$"Processed event '{@event.Message.Payload}' with value '{@event.Event.Value}'\n", cancellationToken);
		}
