...
namespace IO.Eventuate.Tram.IntegrationTests.TestHelpers
{
	public class TestHostBuilder
	{
		...
		private EventuateKafkaConsumerConfigurationProperties _consumerConfigProperties =
		...
		public TestHostBuilder SetConsumerConfigProperties(
		...
		public IHost Build<TConsumerType>(bool withInterceptor) where TConsumerType : class
		{
			_host = new HostBuilder()
				...
				.ConfigureServices((hostContext, services) =>
				{
					...
					services.AddEventuateTramSqlKafkaTransport(_eventuateDatabaseSchemaName, _kafkaBootstrapServers,
						_consumerConfigProperties,
						(provider, o) => { o.UseSqlServer(_sqlConnectionString); });
					if (withInterceptor)
					{
